...
def test_update_user_permission_endpoint_requires_authentication(api_client):
    user_permission_update_url = reverse("user:user-product-permission-detail", kwargs={"pk": 1})
    response = api_client.patch(user_permission_update_url, {})
...
def test_update_user_permission_success(auth_client, request, actor_fixture, target_fixture, region):
    region_initial = region(product_constants.Regions.EU)
    region_updated = region(product_constants.Regions.LATIN_AMERICA)
    actor = request.getfixturevalue(actor_fixture)
    target = request.getfixturevalue(target_fixture)
    permission = UserProductPermissionFactory(user=target, region=region_initial)

    payload = {
        "region": region_updated.id,
    }

    user_permission_update_url = reverse("user:user-product-permission-detail", kwargs={"pk": permission.pk})
    response = auth_client(actor).patch(user_permission_update_url, payload, format="json")

    assert response.status_code == status.HTTP_200_OK

    permission.refresh_from_db()
    assert permission.region == region_updated
...
def test_retrieve_user_permission_if_not_allowed_failure(auth_client, request, actor_fixture, target_fixture, region):
    region_initial = region(product_constants.Regions.EU)
    region_updated = region(product_constants.Regions.LATIN_AMERICA)
    actor = request.getfixturevalue(actor_fixture)
    target = request.getfixturevalue(target_fixture)
