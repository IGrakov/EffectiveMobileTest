...
def test_delete_user_permission_endpoint_requires_authentication(api_client):
    user_permission_delete_url = reverse("user:user-product-permission-detail", kwargs={"pk": 1})
    response = api_client.delete(user_permission_delete_url)
...
def test_delete_user_permission_success(
    auth_client,
    request,
    actor_fixture,
    target_fixture,
    ...
    user_permission_delete_url = reverse("user:user-product-permission-detail", kwargs={"pk": permission.pk})
    response = auth_client(actor).delete(user_permission_delete_url)
...
def test_delete_user_permission_if_not_allowed_failure(
    auth_client,
    request,
    actor_fixture,
    target_fixture,
    ...
    user_permission_delete_url = reverse("user:user-product-permission-detail", kwargs={"pk": permission.pk})
    response = auth_client(actor).get(user_permission_delete_url)
